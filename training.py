...
def load_model(filepath, num_categories):
    model = NeuralNetwork(num_categories).to(device)
    model.load_state_dict(torch.load(filepath, weights_only=True, map_location=device)) # add map_location=torch.device('cpu') to the toch.load() function if using cpu
    return model
